...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg, u8 subAbilityNum)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        gBattleScripting.battler = battler;
            ...
            case ABILITY_SNOW_WARNING:
                ...
                {
                    SetSwitchInDone(battler, subAbilityNum, BattlerSubOrMainAbility(battler, gLastUsedAbility));
                    BattleScriptPushCursorAndCallback(BattleScript_BlockedByPrimalWeatherEnd3);
                    effect++;
                }
                break;
    ...
    case ABILITYEFFECT_OPPORTUNIST:
        ...
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            if (BattlerSubOrMainAbility(battler, ABILITY_OPPORTUNIST))
            {
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        for (u8 i = 0; i < 4; i++) {
            if (i == 0)     partnerDefAbility = GetBattlerAbility(BATTLE_PARTNER(battlerAtk));
            else            partnerDefAbility = GetSubAbilityBySpecies(gBattleMons[BATTLE_PARTNER(battlerAtk)].species, i-1);
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        u32 partnerAtkAbility;
        for (u8 i = 0; i < 4; i++) {
            if (i == 0)     partnerAtkAbility = GetBattlerAbility(BATTLE_PARTNER(battlerAtk));
            else            partnerAtkAbility = GetSubAbilityBySpecies(gBattleMons[BATTLE_PARTNER(battlerAtk)].species, i-1);

            switch (partnerAtkAbility)
            {
            case ABILITY_FLOWER_GIFT:
    ...
    // field abilities
    if (IsAbilityOnField(ABILITY_VESSEL_OF_RUIN) && (atkAbility != ABILITY_VESSEL_OF_RUIN && !BattlerHasSubAbility(battlerAtk, ABILITY_VESSEL_OF_RUIN)) && IsBattleMoveSpecial(move))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));

    if (IsAbilityOnField(ABILITY_TABLETS_OF_RUIN) && (atkAbility != ABILITY_TABLETS_OF_RUIN && !BattlerHasSubAbility(battlerAtk, ABILITY_TABLETS_OF_RUIN)) && IsBattleMovePhysical(move))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));
...
//         return FALSE;
//     else
    return SpeciesHasSubAbility(gBattleMons[battler].species, ability);
}
